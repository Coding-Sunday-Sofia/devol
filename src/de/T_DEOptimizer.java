...
// Import cost
import problem.DEProblem;
// Import screens
import screen.DEScreen;

public class T_DEOptimizer implements Runnable
/***************************************************************
 ** ** D I F F E R E N T I A L E V O L U T I O N ** ** This is the kernel routine
 * for the DE optimization. ** ** Authors: Mikal Keenan ** Kenneth Price **
 ** Rainer Storn ** ** This program implements some variants of Differential **
 ** Evolution (DE) as described in part in the techreport ** tr-95-012.ps of
 * ICSI. You can get this report either via **
 ** ftp.icsi.berkeley.edu/pub/techreports/1995/tr-95-012.ps.Z ** or via WWW:
 * http://http.icsi.berkeley.edu/~storn/litera.html* A more extended version of
 * tr-95-012.ps has appeared in ** the Journal of global optimization. ** ** You
 * may use this program for any purpose, give it to any ** person or change it
 * according to your needs as long as you ** are referring to Rainer Storn and
 * Ken Price as the origi- ** nators of the the DE idea. ** **
 ***************************************************************/
{
	/* ======Public variables====================================== */
	...
	double genbest[] = new double[MaxD]; // the best vector of the current
											// generation
	...
	double rvec[][] = new double[MaxR][MaxD]; // array of randomly chosen
												// vectors
				...
				for (int i = 0; i < NP; i++) {
					...
					do
						rnd[2] = deRandom.nextValue(NP);// BUG:: Changed next()
														// to nextValue
					while ((rnd[2] == i) || (rnd[2] == rnd[1])
							|| (rnd[2] == rnd[0]));

					do
						rnd[3] = deRandom.nextValue(NP);// BUG:: Changed next()
														// to nextValue
					while ((rnd[3] == i) || (rnd[3] == rnd[2])
							|| (rnd[3] == rnd[1]) || (rnd[3] == rnd[0]));

					do
						rnd[4] = deRandom.nextValue(NP);// BUG:: Changed next()
														// to nextValue
					while ((rnd[4] == i) || (rnd[4] == rnd[3])
							|| (rnd[4] == rnd[2]) || (rnd[4] == rnd[1])
							|| (rnd[4] == rnd[0]));

					do
						rnd[5] = deRandom.nextValue(NP);// BUG:: Changed next()
														// to nextValue
					while ((rnd[5] == i) || (rnd[5] == rnd[4])
							|| (rnd[5] == rnd[3]) || (rnd[5] == rnd[2])
					...
					Strategem[current_strategy].apply(F, Cr, dim, trial,
							genbest, rvec);
					...
					if (testcost <= cost[i]) // Better solution than target
												// vectors cost
					{
						assign(g1[i], trial); // if yes put trial vector in new
												// population
	 ...
	 ***********************************************************/
	{
		...
		for (i = 0; i < S.length; i++) {
			try { // give the full pathname here so the class can be located
				Class C = Class.forName("de.DE" + S[i]);
		...
		String Identifier[] = deScreen.getProblemIdentifiers(); // list of
																// problems
		deProblemArray = new DEProblem[Identifier.length]; // Array of problem
															// objects
		// System.out.println("o.k.\n");

		for (i = 0; i < Identifier.length; i++) {// create an object for all
													// problems possible ->
													// improve that!
			try {// Give full path name here so the class can be found
				Class C = Class.forName("problem." + Identifier[i]);
				// System.out.println(C);
				deProblemArray[i] = (DEProblem) C.newInstance(); // select the
																	// problem
				// deProblemArray[i].init (); //set initial size and some
				// constants
			} catch (Exception e) {
	...
	// Let it run! The optimization is taking place here!
	{
		while (action != null) // as long as there is an active thread
		{
			...
			try {
				action.sleep(DEProblem.NAPTIME); // Suspend optimization for
													// NAPTIME
			} catch (Exception E) {
