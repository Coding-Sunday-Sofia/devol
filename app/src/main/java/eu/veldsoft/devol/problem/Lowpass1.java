 ...
 ***********************************************************/
{
	 ...
	 *****************************************************************/
	{
		...
		for (i = 1; i < cz1; i++) // zeroes have a maximum radius of 1(minimum
									// phase)
		{
		...
		for (i = 0; i <= M20; i++) /* stop band */
		{
			...
			y = amag(temp, x, czero, cpole, a0);
			sbub = c2 + (c3 - c2) * (double) i
					/ (double) M20; /* stop band upper bound */
			// ----maximum deviation----------------
			// res = y - sbub;
			// if (res > err) err = res;
			// ----square error---------------------
		...
		/*-------------penalties--------------------------------*/
		/*
		 * res = 0.; for (i=0; i<D; i++) // all parameters must be > 0 { if
		 * (temp[i] < 0) res += 2.e4 - temp[i]*100.; } for (i=1; i < cz1; i++)
		 * // zeroes have a maximum radius of 1(minimum phase) { if (temp[i] >
		 * 1.000000001) res += 100. + 100.*temp[i]; //20 + 10 } for (i=cz1; i <
		 * cz2; i++)// phase angle of zeroes < 0.5 { if (temp[i] > 0.5) res +=
		 * 20. + temp[i]*10.; //20 + 10 }
		 * 
		 * if (cpole > 0) { for (i=cz2; i < czp; i++) // poles have a maximum
		 * radius of 1 { if (temp[i] > 1.) res += 20. + temp[i]*100.; } for
		 * (i=czp; i<D; i++) // poles in stopband doesn't make sense { if
		 * ((temp[i] > 0.375) || (temp[i] < 0.25)) res += 20. + temp[i]*100.; }
		 * }
		 * 
		 * if (res > err) err = res;
		 */
	 ...
	 *****************************************************************/
	{
		...
		/*
		 * -0.039632*Math.cos(r2*2.5) +0.33973*Math.cos(r2*1.5)
