 ...
 * this fonction to place the component -
 *
 ...
 * component be placed just to the right of (for gridx) or just below (for
 * gridy) the component that was added to the container just before this
 * component was added.
 *
 * gridwidth, gridheight Specify the number of columns (for gridwidth) or rows
 * (for gridheight) in the component's display area. These constraints specify
 * the number of cells the component uses, not the number of pixels it uses. The
 * default value is 1. Use GridBagConstraints.REMAINDER to specify that the
 * component be the last one in its row (for gridwidth) or column (for
 * gridheight). Use GridBagConstraints.RELATIVE to specify that the component be
 * the next to last one in its row (for gridwidth) or column (for gridheight).
 *
 * Note: Due to a bug in the the 1.0 release of Java, GridBagLayout doesn't
 * allow components to span multiple rows unless the component is in the
 * leftmost column.
 *
 ...
 *
 * insets Specifies the external padding of the component -- the minimum amount
 * of space between the component and the edges of its display area. The value
 ...
 * where (within the area) to place the component. Valid values are
 ...
 *
 * weightx, weighty Specifying weights is an art that can have a significant
 * impact on the appearance of the components a GridBagLayout controls. Weights
 * are used to determine how to distribute space among columns (weightx) and
 ...
 *
 * Generally weights are specified with 0.0 and 1.0 as the extremes, with
 * numbers in between used as necessary. Larger numbers indicate that the
 * component's row or column should get more space. For each column, the weight
 * is related to the highest weightx specified for a component within that
 * column (with each multi-column component's weight being split somehow between
 * the columns the component is in). Similarly, each row's weight is related to
 * the highest weighty specified for a component within that row. Extra space
...
public class Screen extends Frame {
     ...
     */
    protected void constrain(Container container, Component component,
                             int grid_x, int grid_y, int grid_with, int grid_height, int top,
